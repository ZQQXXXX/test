<template>
    <div id="register">
        <el-card shadow="never"> 
            <div>            
                注册用户
                <el-button @click="$router.push({name:'login'})" icon="el-icon-back"  size="mini" type="info">返回</el-button>
            </div>
        </el-card>
        ...
        methods:{
            add(formtext){
                this.$refs[formtext].validate((valid)=>{
                    if(valid){
                        this.axios.post('/users/regist',this.form).then(()=>{
                            this.$router.push({
                                name:'login',
                            })
                        }).catch(()=>{
